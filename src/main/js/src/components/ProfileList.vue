    ...
    <div class="centered box">
        ...
        <div style="margin-top: 100px; max-height: 75%; overflow-y:scroll; display:block">
            <table class="table" style="width: 90%">
                <tbody>
                <tr v-for="item in filteredData" :key="item.title" @click="viewProfile(item)"
                    ...
                    <td style="width: 500px;font-size: 25px; text-align: left; vertical-align: middle">{{
                            item.title
                        }}
                    </td>
    ...
    methods: {
        ...
        viewProfile(item) {
            this.$router.push({
                name: "Stats",
                params: {id: item.id},
                query: {title: item.title, img: item.base64Image, isPrivate: (item.private ? 'Private chat' : 'Group chat')}
            })
        }
    }
}
...
@import "../assets/styles/profile_list.css";
</style>